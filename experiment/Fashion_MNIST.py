...
X_1 = autoencoder(X_train[:cut_1], stop=True).detach().numpy()
X_2 = autoencoder(X_train[cut_1:cut_2], stop=True).detach().numpy()
X_3 = autoencoder(X_train[cut_2:cut_3], stop=True).detach().numpy()
X_4 = autoencoder(X_train[cut_3:train_num], stop=True).detach().numpy()
X_train_1 = np.concatenate((X_1, X_2), axis=0)
X_train_2 = np.concatenate((X_3, X_4), axis=0)
X_train_full = np.concatenate((X_1, X_2, X_3, X_4), axis=0)
...
Kmeans_index = kmeans.fit_predict(X_train_full)
...
# print("\nspectral clustering")
# time1 = round(time.time()*1000)
# SC_index = sc.fit_predict(X_train_full)
# time2 = round(time.time()*1000)
...
acc = Accuracy(y_true=y_train[:cut_2], y_pred=PSC_index_1)
acc.acc_report()
...
acc = Accuracy(y_true=y_train[cut_2:train_num], y_pred=PSC_index_2)
acc.acc_report()
...
time1 = round(time.time()*1000)
SC_index = sc.fit_predict(X_train_full[:10000])
time2 = round(time.time()*1000)
print(f"SC time spent: {time2 - time1} milliseconds")

acc = Accuracy(y_true=y_train[:10000], y_pred=SC_index)
acc.acc_report()

# PSC + sc: spectral clustering don't have predict() method
psc = PSC(model=model, clustering_method=sc, test_splitting_rate=0.7)
print("\nPSC + sc")
print("\nfit_predict() for 10000 data")
time1 = round(time.time()*1000)
PSC_index_1 = psc.fit_predict(X_train_full[:10000])
time2 = round(time.time()*1000)
print(f"PSC time spent: {time2 - time1} milliseconds")

acc = Accuracy(y_true=y_train[:10000], y_pred=PSC_index_1)
acc.acc_report()